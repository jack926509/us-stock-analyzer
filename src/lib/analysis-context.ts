...
// ─── Local PeerData (mirrors peers route interface) ───────────────────────────
...
const POS_KEYWORDS =
  /beat|record|growth|surge|gain|rise|profit|strong|upgrade|buy|exceed|outperform|launch|new|innovation|partnership|expansion/i
const NEG_KEYWORDS =
  /miss|fall|decline|loss|cut|downgrade|sell|weak|risk|concern|lawsuit|investigation|recall|layoff|debt|warning|disappoints/i
...
export async function buildFinancialSummary(symbol: string): Promise<string> {
  let [income, cashflow, balance] = await Promise.all([
    getIncomeStatements(symbol, "annual", 4).catch(() => []),
    getCashFlowStatements(symbol, "annual", 3).catch(() => []),
    getBalanceSheets(symbol, "annual", 3).catch(() => []),
  ])
  ...
  for (let i = 0; i < Math.min(3, income.length); i++) {
    const inc = income[i] as unknown as Record<string, number> & { date: string }
    const cf = cashflow[i] as unknown as Record<string, number> | undefined
    const bal = balance[i] as unknown as Record<string, number> | undefined
    const prev = income[i + 1] as unknown as Record<string, number> | undefined
    const year = inc.date.substring(0, 4)

    const revYoY =
      prev && (prev.revenue as number) > 0
        ? ` (${(((inc.revenue as number) - (prev.revenue as number)) / (prev.revenue as number) * 100).toFixed(1)}% YoY)`
        : ""
    const niYoY =
      prev && Math.abs(prev.netIncome as number) > 0
        ? ` (${(((inc.netIncome as number) - (prev.netIncome as number)) / Math.abs(prev.netIncome as number) * 100).toFixed(1)}% YoY)`
        : ""
    const gmDelta = prev
      ? ` (${(((inc.grossProfitRatio as number) - (prev.grossProfitRatio as number)) * 100).toFixed(1)}pp YoY)`
      : ""

    lines.push(
      `${year}: 營收 ${fmtB(inc.revenue as number)}${revYoY}, 毛利率 ${((inc.grossProfitRatio as number) * 100).toFixed(1)}%${gmDelta}, ` +
      `淨利率 ${((inc.netIncomeRatio as number) * 100).toFixed(1)}%, 淨利 ${fmtB(inc.netIncome as number)}${niYoY}` +
      (cf ? `, FCF ${fmtB(cf.freeCashFlow as number)}` : "") +
      (bal ? `, 總負債 ${fmtB(bal.totalDebt as number)}, 現金 ${fmtB(bal.cashAndCashEquivalents as number)}` : "")
    )
  }

  if (cashflow.length) {
    const cf0 = cashflow[0] as unknown as Record<string, number> & { date: string }
    const year0 = cf0.date.substring(0, 4)
    const capParts: string[] = []
    if ((cf0.commonStockRepurchased as number) && (cf0.commonStockRepurchased as number) < 0)
      capParts.push(`股票回購 ${fmtB(Math.abs(cf0.commonStockRepurchased as number))}`)
    if ((cf0.dividendsPaid as number) && (cf0.dividendsPaid as number) < 0)
      capParts.push(`股息 ${fmtB(Math.abs(cf0.dividendsPaid as number))}`)
    if (cf0.capitalExpenditure as number)
      capParts.push(`資本支出 ${fmtB(Math.abs(cf0.capitalExpenditure as number))}`)
    if (capParts.length) lines.push(`${year0} 資本配置: ${capParts.join(", ")}`)
  }
...
export async function buildNewsSummary(symbol: string): Promise<string> {
  try {
    ...
    const { data } = await axios.get<
      Array<{ headline: string; summary?: string; datetime: number; url: string }>
...
export async function buildKeyMetricsSummary(symbol: string): Promise<string> {
  ...
  const km = kms[0] as unknown as Record<string, number> | undefined
  const r = ratios[0] as unknown as Record<string, number> | undefined
...
export async function buildPeerSummary(symbol: string): Promise<string> {
  try {
    ...
    const results = await Promise.allSettled(
      allSymbols.map(async (sym): Promise<PeerData | null> => {
        try {
          ...
          const prof =
            profRes.status === "fulfilled" && Array.isArray(profRes.value.data)
              ? profRes.value.data[0]
              : null
          if (!prof) return null
          const km =
            kmRes.status === "fulfilled" && Array.isArray(kmRes.value.data)
              ? kmRes.value.data[0]
              : null
          const rt =
            rtRes.status === "fulfilled" && Array.isArray(rtRes.value.data)
              ? rtRes.value.data[0]
              : null
          const n = (v: unknown) => {
            const x = Number(v)
            return v != null && !isNaN(x) && isFinite(x) && x !== 0 ? x : null
          }
          return {
            symbol: sym,
            companyName: String(prof.companyName ?? sym),
            marketCap: Number(prof.marketCap ?? 0),
            price: Number(prof.price ?? 0),
            sector: String(prof.sector ?? ""),
            isTarget: sym === symbol,
            peRatio: n(km?.peRatio) ?? n(rt?.priceToEarningsRatio),
            pbRatio: n(km?.pbRatio),
            psRatio: n(km?.psRatio),
            evToEbitda: n(km?.evToEbitda) ?? n(rt?.enterpriseValueMultiple),
            roe: n(km?.roe) ?? n(rt?.returnOnEquity),
            grossMargin: n(rt?.grossProfitMargin),
            revenueGrowth: null,
          }
        } catch {
          return null
    ...
    const peers = results
      ...
      .filter((p): p is PeerData => p !== null)
    if (!peers.length) return "（無同業數據）"

    const fmt = (v: number | null, d = 1, sfx = "x") =>
      v != null ? v.toFixed(d) + sfx : "N/A"
    const fmtPct = (v: number | null) =>
      v != null ? (v * 100).toFixed(1) + "%" : "N/A"
    const fmtCap = (v: number) =>
      v >= 1e12 ? `$${(v / 1e12).toFixed(1)}T` : v >= 1e9 ? `$${(v / 1e9).toFixed(1)}B` : "N/A"
...
export async function buildAnalystConsensusSummary(symbol: string): Promise<string> {
  try {
    ...
    if (
      estRes.status === "fulfilled" &&
      Array.isArray(estRes.value.data) &&
      estRes.value.data.length
    ) {
      ...
      if (epsAvg)
        parts.push(
          `下期預期EPS: $${epsAvg.toFixed(2)} (區間 $${epsLow?.toFixed(2) ?? "N/A"} – $${epsHigh?.toFixed(2) ?? "N/A"})`
        )
    }
...
// ─── Main context builder (called once, shared across all persona prompts) ────

export async function buildAnalysisContext(symbol: string): Promise<AnalysisContext> {
  const [
    fmpProfile,
    fmpQuote,
    financialSummary,
  ...
  const marketCap =
    fmpProfile?.marketCap && fmpProfile.marketCap > 0
  ...
  const range52w =
    yearHigh && yearLow ? `$${yearLow.toFixed(2)} – $${yearHigh.toFixed(2)}` : "N/A"
